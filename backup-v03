...
# SCRIPT DESENVOLVIDO PELA INTELIGENCIA ORGANICA DE FRANCO COM AJUDA DA INTELIGENCIA ARTIFICIAL DO DUCK.AI
#
...
# O script também espera que a pasta onde estão os dados esteja mapeada no docker-compose.yaml
...
MAX_BACKUP_COPIES=10      # Manter no máximo 10 cópias de backup (manter os mais recentes)
...
CONTAINER_CHECK_RETRIES=3 # Número de tentativas para iniciar container
...
if [[ -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
  ...
  log "ERROR: docker-compose.yml ou docker-compose.yaml não encontrado em: $SCRIPT_DIR"
  exit 1
fi
...
validate_compose_config() {
  if command -v docker-compose >/dev/null 2>&1; then
    if ! docker-compose config >/dev/null 2>&1; then
      log "ERROR: Configuração docker-compose inválida"
      exit 1
    fi
    ...
    log "ERROR: docker ou docker-compose não encontrado"
    exit 1
  fi
}
...
get_data_dir_from_compose() {
  ...
  if [[ -z "$data_dir" ]]; then
    log "ERROR: Volume para /data não encontrado em: $COMPOSE_FILE"
...
if [[ -z "${CONTAINER_NAME:-}" ]]; then
  log "ERROR: 'container_name:' não encontrado em: $COMPOSE_FILE"
  exit 1
fi
...
require_file() {
  local f="$1"
  if [[ ! -f "$f" ]]; then
    log "ERROR: arquivo obrigatório não encontrado: $f"
    exit 1
  fi
}
...
if [[ -z "${RCON_PASSWORD:-}" ]]; then
  log "ERROR: rcon.password não encontrado ou vazio em: $SERVER_PROPERTIES_FILE"
  exit 1
fi
if [[ -z "${RCON_PORT:-}" ]]; then
  log "ERROR: rcon.port não encontrado ou vazio em: $SERVER_PROPERTIES_FILE"
  exit 1
fi

if [[ ! -d "$WORLD_SRC" ]]; then
  log "ERROR: diretório world não existe: $WORLD_SRC"
  exit 1
fi
...
# Aguarda container iniciar com sucesso (com retries)
wait_container_ready() {
  ...
  while (( attempt < retries )); do
    ...
    ((attempt++))
    log "Tentativa $((attempt)) de $retries aguardando container iniciar..."
  done

  log "ERROR: Container falhou ao iniciar após $retries tentativas"
  return 1
}
...
players_online_list() {
  ...
  # Tenta extrair lista de players da resposta RCON
...
run_step() {
  ...
  if [[ $rc -eq 0 ]]; then
    ...
    printf "\r✗ %s\n" "$msg"
    log "---- output do comando (últimas 50 linhas) ----"
...
restart_without_backup() {
  ...
  if [[ "$player_count" =~ ^[0-9]+$ ]] && (( player_count > 0 )); then
    ...
    log "Nenhum player online - pulando contagem regressiva, iniciando backup imediatamente"
  fi
  ...
  if wait_container_ready "$CONTAINER_CHECK_RETRIES"; then
    ...
    log "ERROR: Falha ao reiniciar container"
    exit 1
  fi
}
...
backup_with_restart() {
  ...
  if wait_container_ready "$CONTAINER_CHECK_RETRIES"; then
    ...
    log "ERROR: Falha ao reiniciar container após backup"
    exit 1
  fi
}
...
if ! test_rcon; then
  log "ERROR: Falha ao conectar ao RCON em ${CONTAINER_NAME}:${RCON_PORT}"
  exit 1
fi
